...
\begin{equation}
F(\vec{r}_{ij}): \mathbb{V} \rightarrow \mathbb{R}^2 
\end{equation}
la cui forza agente tra due individui i,j a distanza r e aventi la medesima opinione è
\begin{equation}
    \vec{F_{ij}} = \dfrac{Gm_im_j}{r^2} \hat{r}
\end{equation}
nell'ipotesi in cui tutti gli individui hanno massa m=1 e che la costante gravitazionale fosse G=10, per facilitare l'implementazione del modello. \\ L'introduzione di un campo di questo tipo favorisce il \textit{flocking}, ossia la formazione di assembramenti tra individui della stessa opinione. Questa richiesta permette di dare un senso alla dinamica degli individui nello spazio: come si osserva in una comunità reale, persone aventi opinione politica affine, tendono a stare assieme mentre persone immerse in un ambiente in cui la propria ideologia politica non è ben vista, tendono a spostarsi dove ci sono individui con la propria opinione. \\
Avendo introdotto delle \textit{periodic boundary conditions}, la dinamica degli individui vive sul toroide \Zii. Questo può rappresentare un problema ai fini della dinamica, infatti ci sono sempre due possibili distanze tra due qualsiasi individui e la forza di attrazione gravitazionale deve fare una scelta per decidere il suo modulo, direzione e verso. 
...
\addplot3
[domain=0:360, y domain=0:360, variable=\u, variable y=\v, mesh, samples=50, z buffer=sort, surf, colormap/blackwhite]
...
\caption{\textit{Esempio di caso particolare in cui la forza agente tra i due individui in rosso è nulla. A sinistra la rappresentazione nell'automa cellulare di questa situazione, a destra la sua corrispondenza sul toroide \Zii che tiene conto delle periodic boundary conditions.}}
\end{figure}
