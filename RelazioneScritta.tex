...
\end{aligned}
\end{equation}
la cui forza agente tra due individui (i,j), a distanza $\vec{r}_{x}$ lungo le ascisse e $\vec{r}_{y}$ lungo le ordinate, aventi la medesima opinione è
...
\end{tikzpicture}
\caption{\textit{Rappresentazione grafica delle possibili combinazione di coppie di forze agenti su un individuo}}.
\label{Fig:7}
\end{figure}
L'introduzione di campi di questo tipo favorisce il fenomeno del \textit{flocking}, ossia la formazione di assembramenti tra individui della stessa opinione. Questo fenomeno introduce degli attrattori nella dinamica degli individui nello spazio \Zii: come si osserva in una comunità reale, persone aventi opinione politica affine tendono a stare assieme mentre, persone immerse in un ambiente in cui la propria ideologia politica non è ben vista, tendono a spostarsi dove ci sono individui che condividono la propria opinione. %\bigskip
\\
Avendo introdotto delle condizioni al contorno periodiche, la dinamica degli individui vive sul toroide \Zii in quanto le linee di forza dei campi $F_x, F_y$ stanno sulla superficie del toroide (vedi Figura \ref{fig:3}).\\ Questo può rappresentare un problema ai fini della dinamica, infatti per ognuna delle due forze agenti tra due individui ci sono sempre due possibili distanze ma è necessario che il modulo, direzione e verso di queste forze siano univocamente definiti per ogni coppia di individui (i,j). Ad esempio, se consideriamo solo le forze agenti in direzione x, per ogni coppia di individui (i,j) esiste una distanza (e quindi una forza) che segue il verso positivo dell'asse delle ascisse, che possiamo chiamare $r_x^+$, e una che va in senso opposto che chiamiamo $r_x^-$. Una considerazione del tutto analoga può essere fatta per le forze agenti in y. Per questo motivo, nel calcolare la distanza dalla quale dipendono le forze $\vec{F}^{(i,j)}_x$ e $\vec{F}^{(i,j)}_y$, sono state scelte le distanze $r_x(i,j)$ ed $r_y(i,j)$ in modo da soddisfare le condizioni
\begin{align*} 
&r_x(i,j) = min\{|r_x^+|, |r_x^-|\}\\
&r_y(i,j) = min\{|r_y^+|, |r_y^-|\}.\\
\end{align*}
Chiaramente è anche necessario introdurre delle opportune condizioni sulle forze per evitare dei punti singolari. Inanzitutto si assume che se $r_x(i,j)$ o $r_y(i,j)$ sono uguali a zero, allora la forza corrispondente in y o in x sarà nulla. È evidente che non è possibile avere entrambe $r_x(i,j)$ e $r_y(i,j)$ uguali a zero perchè questo va in contraddizione con la richiesta $i\neq j$ (gli individui non si possono compenetrare). Allora viene subito da domandarsi quando sia $\vec{F}^{(i,j)}_x$ che $\vec{F}^{(i,j)}_y$ possano essere entrambe nulle: questo caso particolare richiede delle considerazioni di carattere geometrico che risiedono nella struttura del toroide topologico. Quando la condizione $r_x(i,j) = min\{|r_x^+|, |r_x^-|\}$ ammette come soluzione sia $r_x^+$ che $r_x^-$ (ovvero quando le due distanze sono uguali in modulo), si richiede $\vec{F}^{(i,j)}_x$ = 0. Ne è un esempio la Figura \ref{Fig:8}.
...
La scelta del movimento degli individui all'interno del proprio intorno unitario di Von Neumann, viene quindi condizionata dai campi $F_x,F_y$ e tende a minimizzare la distanza tra gli individui di egual opinione, in modo da formare delle regioni ad alta densità di individui e regioni a bassa densità di individui. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
MATTEQUISEVUOIMETTIQUALCOSASULLASCELTADEIMOVIMENTI

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Questo ha effetti notevoli sulla dinamica delle opinioni in quanto le due fazioni corrispondenti ai due stati \textit{s} intraprendono una specie di ''guerra di logoramento'' in cui gli individui si dispongono in ''trincee'' e cercano di far prevalere la propria opinione (Vedi Fig.\ref{Fig:9}). 
\begin{figure}[h]
\centering
